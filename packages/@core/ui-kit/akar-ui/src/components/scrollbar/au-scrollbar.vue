...
import type { ClassType } from '@mandor-core/typings';

import { computed, ref } from 'vue';

import { BaseScrollArea, BaseScrollBar } from '../../base/scroll-area';
...
<template>
  ...
  >
    <div
      ...
      class="scrollbar-top-shadow will-change-opacity opacity-0 h-12 w-full pointer-events-none transition-opacity-300 ease-in-out top-0 absolute z-10"
    />
    ...
    <div
      ...
      class="scrollbar-bottom-shadow will-change-opacity opacity-0 h-12 w-full pointer-events-none transition-opacity-300 ease-in-out bottom-0 absolute z-10"
    />
...
.scrollbar-top-shadow {
  background: linear-gradient(
    to bottom,
    hsl(var(--scroll-shadow, var(--color-background))),
    transparent
  );
}
